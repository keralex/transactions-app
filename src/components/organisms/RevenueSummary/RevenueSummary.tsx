    ...
    return (
        <div className='bg-white '>
            <Container className=" bg-background-base p-4 pt-20 border-r border-[#DEE2EC] rounded-tr-[28px] border-tl-transparent  " isFlex direction="column" gap={4}>
                <Text as='h1' variant='h1' className='text-text-title pl-9  mb-4' weight='semibold'>Tus cobros</Text>
                {!hasCustomFilters &&
                    <div className="grid grid-cols-3 gap-0 text-sm font-medium w-full  mb-4">
                        {options.map((opt) => (
                            <button
                                key={opt.value}
